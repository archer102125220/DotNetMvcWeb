...
{
    public class OracleDemoCategoryControllerTests
    {
        private readonly Mock<IOracleDemoCategoryService> _categoryServiceMock;
        private readonly OracleDemoCategoryController _controller;
        private readonly DefaultHttpContext _httpContext;

        public OracleDemoCategoryControllerTests()
        {
            _categoryServiceMock = new Mock<IOracleDemoCategoryService>();
            _controller = new OracleDemoCategoryController(_categoryServiceMock.Object);

            _httpContext = new DefaultHttpContext();
            Mock<IUrlHelper> urlHelperMock = new();
            urlHelperMock.Setup(u => u.Action(It.IsAny<UrlActionContext>())).Returns("/OracleDemoCategory");
        ...
        {
            List<OracleDemoCategory> categories = new() { new OracleDemoCategory { Id = 1, Name = "Cat1" } };
        ...
        {
            _categoryServiceMock.Setup(s => s.GetCategoriesAsync()).ReturnsAsync(new List<OracleDemoCategory>());
            IActionResult result = await _controller.Create();
            ViewResult view = Assert.IsType<ViewResult>(result);
        ...
        {
            OracleDemoCategory item = new() { Name = "NewCat" };
        ...
        {
            OracleDemoCategory item = new() { Name = "" };
            ...
            PartialViewResult partial = Assert.IsType<PartialViewResult>(result);
            Assert.Equal("_CreateOrEdit", partial.ViewName);
            Assert.Equal("#oracle-demo-category-form-container", _httpContext.Response.Headers["HX-Retarget"].ToString());
        }
        ...
        {
            _categoryServiceMock.Setup(s => s.GetCategoryByIdAsync(999)).ReturnsAsync((OracleDemoCategory?)null);
        ...
        {
            OracleDemoCategory cat = new() { Id = 2, Name = "Cat2" };
        ...
        {
            OracleDemoCategory cat = new() { Id = 2, Name = "" };
            _controller.ModelState.AddModelError("Name", "Required");
