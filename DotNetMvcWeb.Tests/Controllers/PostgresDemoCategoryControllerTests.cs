...
{
    public class PostgresDemoCategoryControllerTests
    {
        private readonly Mock<IPostgresDemoCategoryService> _categoryServiceMock;
        private readonly PostgresDemoCategoryController _controller;
        private readonly DefaultHttpContext _httpContext;

        public PostgresDemoCategoryControllerTests()
        {
            _categoryServiceMock = new Mock<IPostgresDemoCategoryService>();
            _controller = new PostgresDemoCategoryController(_categoryServiceMock.Object);

            _httpContext = new DefaultHttpContext();
            Mock<IUrlHelper> urlHelperMock = new();
            urlHelperMock.Setup(u => u.Action(It.IsAny<UrlActionContext>())).Returns("/PostgresDemoCategory");
        ...
        {
            List<PostgresDemoCategory> categories = new() { new PostgresDemoCategory { Id = 1, Name = "Cat1" } };
        ...
        {
            _categoryServiceMock.Setup(s => s.GetCategoriesAsync()).ReturnsAsync(new List<PostgresDemoCategory>());
            IActionResult result = await _controller.Create();
            ViewResult view = Assert.IsType<ViewResult>(result);
        ...
        {
            PostgresDemoCategory item = new() { Name = "NewCat" };
        ...
        {
            PostgresDemoCategory item = new() { Name = "" };
            ...
            PartialViewResult partial = Assert.IsType<PartialViewResult>(result);
            Assert.Equal("_CreateOrEdit", partial.ViewName);
            Assert.Equal("#postgres-demo-category-form-container", _httpContext.Response.Headers["HX-Retarget"].ToString());
        }
        ...
        {
            _categoryServiceMock.Setup(s => s.GetCategoryByIdAsync(999)).ReturnsAsync((PostgresDemoCategory?)null);
        ...
        {
            PostgresDemoCategory cat = new() { Id = 2, Name = "Cat2" };
        ...
        {
            PostgresDemoCategory cat = new() { Id = 2, Name = "" };
            _controller.ModelState.AddModelError("Name", "Required");
