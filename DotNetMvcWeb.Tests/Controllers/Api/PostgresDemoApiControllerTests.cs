...
{
    public class PostgresDemoApiControllerTests
    {
        private readonly Mock<IPostgresDemoItemService> _serviceMock;
        private readonly PostgresDemoApiController _controller;

        public PostgresDemoApiControllerTests()
        {
            _serviceMock = new Mock<IPostgresDemoItemService>();
            _controller = new PostgresDemoApiController(_serviceMock.Object);
        }

        [Fact]
        public async Task GetItems_ReturnsOkWithItems()
        {
            List<PostgresDemoItem> items = new() { new PostgresDemoItem { Id = 1, Name = "Item1" } };
            _serviceMock.Setup(s => s.GetItemsAsync("key")).ReturnsAsync(items);

            ActionResult<IEnumerable<PostgresDemoItem>> result = await _controller.GetItems("key");
        ...
        public async Task GetItem_WhenExists_ReturnsOkWithItem()
        {
            ...
            ActionResult<PostgresDemoItem> result = await _controller.GetItem(1);
        ...
        {
            _serviceMock.Setup(s => s.GetItemByIdAsync(999, true)).ReturnsAsync((PostgresDemoItem?)null);

            ActionResult<PostgresDemoItem> result = await _controller.GetItem(999);
        ...
        {
            PostgresDemoItem item = new() { Id = 10, Name = "NewItem" };
            _serviceMock.Setup(s => s.CreateItemAsync(item)).Returns(Task.CompletedTask);

            ActionResult<PostgresDemoItem> result = await _controller.CreateItem(item);

            CreatedAtActionResult created = Assert.IsType<CreatedAtActionResult>(result.Result);
            Assert.Equal(nameof(PostgresDemoApiController.GetItem), created.ActionName);
        ...
        {
            PostgresDemoItem item = new() { Id = 5, Name = "Item5" };
        ...
        public async Task GetItemsViaAdoNet_WhenSuccess_ReturnsOkList()
        {
            ...
            ActionResult<IEnumerable<PostgresDemoItem>> result = await _controller.GetItemsViaAdoNet("test");
