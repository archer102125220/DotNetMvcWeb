...
namespace DotNetMvcWeb.Services.Interfaces
{
    ...
    /// </summary>
    public interface IPostgresDemoItemService
    {
        Task<List<PostgresDemoItem>> GetItemsAsync(string? keyword = null);
        Task<PostgresDemoItem?> GetItemByIdAsync(int id, bool includeCategory = false);
        Task CreateItemAsync(PostgresDemoItem item);
        Task UpdateItemAsync(PostgresDemoItem item);
