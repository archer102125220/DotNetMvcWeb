...
* **開發流程**：當您想新增一個資料表或欄位時，您必須親手下達指令建立一個空白的 Migration 檔，然後**手動寫入** `up` (新增) 與 `down` (復原) 的資料庫腳本（例如 Laravel 的 `$table->string('name')` 或是 Sequelize 的 `queryInterface.addColumn(...)`）。
* **框架的認知**：框架本身其實**不知道**您的資料庫「完整的長相」是什麼，它只是把您手寫的腳本，按照時間順序一個個執行而已。
...
* **開發流程**：您完全不需要手寫資料庫異動腳本！您只需要專心在 C# 裡寫好您的 `class`，加上對應的屬性 (Property) 與約束標記 (例如 `[Required]`)。
* **黑魔法的關鍵**：當您下達 `dotnet ef migrations add` 時，EF Core 會把「**您現在剛寫好的 C# class**」拿去跟「**上次存下來的 Snapshot (.Designer.cs)**」進行交叉比對 (Diff)。
* **自動產出**：它算出這兩者之間的差異後，**「自動」**幫您寫出 `Up()` 與 `Down()` 的指令檔。
...
> 💡 **相關閱讀**：如果您想了解 EF Core 的實務操作指令，以及如何避免 Migration 過程中的各種雷區，請參考 [Entity Framework Core (EF Core) 實戰教學指南](./ef-core-orm-guide.md)。
